# Custom Partitioning Key[¶](https://clickhouse.tech/docs/en/engines/table-engines/mergetree-family/custom-partitioning-key/#custom-partitioning-key)

Partitioning is available for the [MergeTree](https://clickhouse.tech/docs/en/engines/table-engines/mergetree-family/mergetree/) family tables (including [replicated](https://clickhouse.tech/docs/en/engines/table-engines/mergetree-family/replication/) tables). [Materialized views](https://clickhouse.tech/docs/en/engines/table-engines/special/materializedview/#materializedview) based on MergeTree tables support partitioning, as well.
...
The partition key can also be a tuple of expressions (similar to the [primary key](https://clickhouse.tech/docs/en/engines/table-engines/mergetree-family/mergetree/#primary-keys-and-indexes-in-queries)). For example:
...
Use the [system.parts](https://clickhouse.tech/docs/en/operations/system-tables/parts/#system_tables-parts) table to view the table parts and partitions. For example, let’s assume that we have a `visits` table with partitioning by month. Let’s perform the `SELECT` query for the `system.parts` table:
...
The `partition` column contains the names of the partitions. There are two partitions in this example: `201901` and `201902`. You can use this column value to specify the partition name in [ALTER … PARTITION](https://clickhouse.tech/docs/en/engines/table-engines/mergetree-family/custom-partitioning-key/#alter_manipulations-with-partitions) queries.

The `name` column contains the names of the partition data parts. You can use this column to specify the name of the part in the [ALTER ATTACH PART](https://clickhouse.tech/docs/en/engines/table-engines/mergetree-family/custom-partitioning-key/#alter_attach-partition) query.
...
The `detached` directory contains parts that were detached from the table using the [DETACH](https://clickhouse.tech/docs/en/sql-reference/statements/alter/partition/#alter_detach-partition) query. The corrupted parts are also moved to this directory, instead of being deleted. The server does not use the parts from the `detached` directory. You can add, delete, or modify the data in this directory at any time – the server will not know about this until you run the [ATTACH](https://clickhouse.tech/docs/en/sql-reference/statements/alter/partition/#alter_attach-partition) query.
...
ClickHouse allows you to perform operations with the partitions: delete them, copy from one table to another, or create a backup. See the list of all operations in the section [Manipulations With Partitions and Parts](https://clickhouse.tech/docs/en/sql-reference/statements/alter/partition/#alter_manipulations-with-partitions).
