# Distributed Table Engine[ ](https://clickhouse.tech/docs/en/engines/table-engines/special/distributed/#distributed)
...
- (optionally) policy name, it will be used to store temporary files for async send
  ...
  - [insert_distributed_sync](https://clickhouse.tech/docs/en/operations/settings/settings/#insert_distributed_sync) setting
  - [MergeTree](https://clickhouse.tech/docs/en/engines/table-engines/mergetree-family/mergetree/#table_engine-mergetree-multiple-volumes) for the examples
...
\- `user` – Name of the user for connecting to a remote server. Default value: default. This user must have access to connect to the specified server. Access is configured in the users.xml file. For more information, see the section [Access rights](https://clickhouse.tech/docs/en/operations/access-rights/).
...
When specifying replicas, one of the available replicas will be selected for each of the shards when reading. You can configure the algorithm for load balancing (the preference for which replica to access) – see the [load_balancing](https://clickhouse.tech/docs/en/operations/settings/settings/#settings-load_balancing) setting.
...
The Distributed engine requires writing clusters to the config file. Clusters from the config file are updated on the fly, without restarting the server. If you need to send a query to an unknown set of shards and replicas each time, you don’t need to create a Distributed table – use the `remote` table function instead. See the section [Table functions](https://clickhouse.tech/docs/en/sql-reference/table-functions/).
...
Data is written asynchronously. When inserted in the table, the data block is just written to the local file system. The data is sent to the remote servers in the background as soon as possible. The period for sending data is managed by the [distributed_directory_monitor_sleep_time_ms](https://clickhouse.tech/docs/en/operations/settings/settings/#distributed_directory_monitor_sleep_time_ms) and [distributed_directory_monitor_max_sleep_time_ms](https://clickhouse.tech/docs/en/operations/settings/settings/#distributed_directory_monitor_max_sleep_time_ms) settings. The `Distributed` engine sends each file with inserted data separately, but you can enable batch sending of files with the [distributed_directory_monitor_batch_inserts](https://clickhouse.tech/docs/en/operations/settings/settings/#distributed_directory_monitor_batch_inserts) setting. This setting improves cluster performance by better utilizing local server and network resources. You should check whether data is sent successfully by checking the list of files (data waiting to be sent) in the table directory: `/var/lib/clickhouse/data/database/table/`. The number of threads performing background tasks can be set by [background_distributed_schedule_pool_size](https://clickhouse.tech/docs/en/operations/settings/settings/#background_distributed_schedule_pool_size) setting.
...
When the `max_parallel_replicas` option is enabled, query processing is parallelized across all replicas within a single shard. For more information, see the section [max_parallel_replicas](https://clickhouse.tech/docs/en/operations/settings/settings/#settings-max_parallel_replicas).

## Virtual Columns[ ](https://clickhouse.tech/docs/en/engines/table-engines/special/distributed/#virtual-columns)

- `_shard_num` — Contains the `shard_num` (from `system.clusters`). Type: [UInt32](https://clickhouse.tech/docs/en/sql-reference/data-types/int-uint/).

Note

Since [`remote`](https://clickhouse.tech/docs/en/sql-reference/table-functions/remote/)/`cluster` table functions internally create temporary instance of the same Distributed engine, `_shard_num` is available there too.

**See Also**

- [Virtual columns](https://clickhouse.tech/docs/en/engines/table-engines/special/#table_engines-virtual_columns)
- [background_distributed_schedule_pool_size](https://clickhouse.tech/docs/en/operations/settings/settings/#background_distributed_schedule_pool_size)

[Original article](https://clickhouse.tech/docs/en/operations/table_engines/distributed/)