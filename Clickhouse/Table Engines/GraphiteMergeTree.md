...
- Without thinning.
  ...
  Uses the [MergeTree](https://clickhouse.yandex/docs/en/operations/table_engines/mergetree/#table_engines-mergetree) engine.

- With thinning.

  Using the `GraphiteMergeTree` engine.

The engine inherits properties from MergeTree. The settings for thinning data are defined by the [graphite_rollup](https://clickhouse.yandex/docs/en/operations/server_settings/settings/#server_settings-graphite_rollup) parameter in the server configuration.
