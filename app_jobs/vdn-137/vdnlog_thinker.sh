...
hive -e "INSERT INTO TABLE VDN_AVLB_DAILY PARTITION (ds = '${current}') SELECT v.ERR, COUNT(v.ERR) AS TOTAL, '${current}'  FROM VDN_LOGS v WHERE v.DS = '${current}' AND v.ERR IN ('208000', '303000', '301010', '301020', '301030', '304001', '304002', '304003', '304004', '601000', '602000', '100000', '110000') OR v.ERR  LIKE '301030_%' GROUP BY v.CT, v.ERR ;"


echo "insertting data into table VDN_AVLB_MINUTELY ..."
hive -e "INSERT INTO TABLE VDN_AVLB_MINUTELY PARTITION (ds = '${current}') SELECT v.ERR, COUNT(v.ERR) AS TOTAL, '${current}', v.TR FROM VDN_LOGS v WHERE v.DS = '${current}' AND v.ERR IN ('208000', '303000', '301010', '301020', '301030', '304001', '304002', '304003', '304004', '601000', '602000', '100000', '110000') OR v.ERR  LIKE '301030_%' GROUP BY v.ERR, v.TR ;"

echo "insertting data into table VDN_AVLB_MINUTELY_FULLY ..."
hive -e "INSERT INTO TABLE VDN_AVLB_MINUTELY_FULLY PARTITION (ds = '${current}') SELECT v.ERR, COUNT(v.ERR) AS TOTAL, v.ISP, v.COUNTRY, v.CITY, '${current}', v.TR, v.CAT FROM VDN_LOGS_FULLY v WHERE v.DS = '${current}' AND v.ERR IN ('208000', '303000', '301010', '301020', '301030', '304001', '304002', '304003', '304004', '601000', '602000', '100000', '110000') OR v.ERR  LIKE '301030_%' GROUP BY v.ERR, v.ISP, v.COUNTRY, v.CITY, v.TR, v.CAT ;"
