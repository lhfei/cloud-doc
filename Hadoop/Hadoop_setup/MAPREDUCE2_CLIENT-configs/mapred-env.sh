      ...
      export HADOOP_OPTS="-Djava.io.tmpdir=/var/lib/hadoop/data/tmp/hadoop_java_io_tmpdir $HADOOP_OPTS"
      export JAVA_LIBRARY_PATH="${JAVA_LIBRARY_PATH}:/var/lib/hadoop/data/tmp/hadoop_java_io_tmpdir"

      # History server logs
      export HADOOP_LOG_DIR=/export/var/log/hadoop-mapreduce/$USER

      # History server pid
      export HADOOP_PID_DIR=/export/var/run/hadoop-mapreduce/$USER